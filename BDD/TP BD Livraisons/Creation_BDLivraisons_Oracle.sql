-- script creation BD Livraisons

CREATE TABLE CategoriesClt(
  Classe CHAR(1) PRIMARY KEY,
  Reduction NUMBER(2,0) NOT NULL);

CREATE TABLE Clients(
  NumClt NUMBER(5,0) PRIMARY KEY,
  NomClt VARCHAR2(20)  NOT NULL,
  Categorie CHAR(1) NULL REFERENCES CategoriesClt(Classe));

CREATE TABLE Produits(
  NumProduit NUMBER(5,0) PRIMARY KEY,
  Libelle VARCHAR2(20),
  PrixBrut NUMBER(5,0));

CREATE TABLE Livraisons(
  NumProduit NUMBER(5,0) REFERENCES Produits(NumProduit),
  NumClt NUMBER(5,0) REFERENCES Clients(NumClt),
  Quantite NUMBER(6,0) NOT NULL,
  PRIMARY KEY (NumProduit, NumClt));

CREATE TABLE Composition(
  NumProCompose NUMBER(5,0) REFERENCES Produits(NumProduit),
  NumProComposant NUMBER(5,0) REFERENCES Produits(NumProduit),
  Quantite NUMBER(6,2) NOT NULL,
